...
int rayTriangleIntersection(CompFab::Ray &ray, CompFab::Triangle &triangle)
{
    ...
    // referenced from http://www.cs.virginia.edu/~gfx/Courses/2003/ImageSynthesis/papers/Acceleration/Fast%20MinimumStorage%20RayTriangle%20Intersection.pdf
    
    double det, inv_det, t, u, v;
    
    /* find vectors for two edges sharing vertex_1 */
    CompFab::Vec3 edge1(triangle.m_v2 - triangle.m_v1);
    CompFab::Vec3 edge2(triangle.m_v3 - triangle.m_v1);
    
    /* begin calculating determinant - also used to calculate U parameter */
    CompFab::Vec3 pvec = ray.m_direction % edge2;
    ...
    /* calculate distance from vert0 to ray origion */
    CompFab::Vec3 tvec = ray.m_origin - triangle.m_v1;
    ...
    /* prepare to vest V parameter */
    CompFab::Vec3 qvec = tvec % edge1;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "      Voxel Grid Information : " << std::endl;
    std::cout << "-------------------------------------" << std::endl;
    std::cout << "m_lowerLeft\t: " << g_voxelGrid->m_lowerLeft[0] << " " << g_voxelGrid->m_lowerLeft[1]<< " " << g_voxelGrid->m_lowerLeft[2] << std::endl;
    std::cout << "m_dimX\t\t: "      << g_voxelGrid->m_dimX << std::endl;
    std::cout << "m_dimY\t\t: "      << g_voxelGrid->m_dimY << std::endl;
    std::cout << "m_dimZ\t\t: "      << g_voxelGrid->m_dimZ << std::endl;
    std::cout << "m_size\t\t: "      << g_voxelGrid->m_size << std::endl;
    std::cout << "m_spacing\t: "   << g_voxelGrid->m_spacing << std::endl;
    
    std::cout << "voxelization starts .........." << std::endl;
    ...
    for (int _x = 0; _x < g_voxelGrid->m_dimX; _x++)
    {
        for (int _y = 0; _y < g_voxelGrid->m_dimY; _y++)
        {
            for (int _z = 0; _z < g_voxelGrid->m_dimZ; _z++)
            {
                CompFab::Vec3 vPos((double) _x * spacing, (double) _y * spacing, (double) _z * spacing);
                if(numSurfaceIntersections(vPos, direction) % 2 != 0)
                {
                    count++;
                    g_voxelGrid->isInside(_x,_y,_z) = 1;
                    //std::cout << _x << ", " << _y << ", " << _z << " : In" << std::endl;
                }
            }
        }
    }
    ...
    printf("naive method : active voxels found %d points in %.5f sec.\n", count, T);
